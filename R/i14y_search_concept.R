...
i14y_search_concept <- function (query = NULL, language = "de", page = 1, pageSize = 1000, 
                                 publishers = NULL, themes = NULL,conceptValueTypes=NULL,registrationStatuses=NULL) 
{
  ...
  if (!is.null(conceptValueTypes)) {
    ...
    conceptValueTypes <- rlang::arg_match(conceptValueTypes, valid_concept_value_types)
  }
  

  if (!is.null(registrationStatuses)) {
    valid_registration_statuses <- c(
      "Superseded", "PreferredStandard", "Incomplete", "Candidate",
      "Qualified", "Recorded", "Standard"
    )
    registrationStatuses <- rlang::arg_match(registrationStatuses, valid_registration_statuses)
  }
  ...
  req <- httr2::req_url_query(req, language = language, page = page, 
                              pageSize = pageSize, query = query, publishers = publishers, 
                              themes = themes,types = "Concept",conceptValueTypes=conceptValueTypes,registrationStatuses=registrationStatuses)
